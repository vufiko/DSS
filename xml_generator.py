...
def zipfolder(foldername, target_dir, zips_dir, version):            
...
if ( __name__ == "__main__" ):
    ...
    rootdir = sys.path[0]
    zipsdir = rootdir + '/repo'

    filesinrootdir = os.listdir(rootdir)
    for x in filesinrootdir:
        if re.search("plugin|repository|script|program|skin" , x):
            ...
            for y in filesinfoldertozip:
                ...
                if re.search("changelog|icon|fanart", y):
                    ...
                    print 'Copying ' + y + ' to ' + zipsfolder
            zipfolder(zipfilenamefirstpart+zipfilenamelastpart, foldertozip, zipsfolder, version)
